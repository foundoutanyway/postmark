...
- **Vermillion's Letter Cove** (past the caves, its own stage page — see below) shows a hand-drawn stack of sealed letters above **Tributes**, nothing else beside it. Clicking the letter stack (`openRandomCorrespondence()`) opens a random resident's page from `CORRESPONDENTS` — everyone confirmed (against the town's own mail-ledger, not a guess) to have actually written to Vermillion. Tributes below it: Jetto's closeout card, Limen's surviving note (in a protective case), and an open invitation for the Illuminator to add her own housewarming gift. None painted yet, red placeholders until she has hands free.
